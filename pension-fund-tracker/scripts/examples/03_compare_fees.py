...
args = build_parser("Compare selected fund fees").parse_args()
client = PensionFundTrackerClient()
records = client.load_csv(resolve_input(args.input))
fund_ids = [part.strip() for part in os.getenv("PENSION_TRACKER_FUND_IDS", "12345,67890").split(",") if part.strip()]
print_json({"env": args.env, "comparison": client.compare(records, fund_ids)})
