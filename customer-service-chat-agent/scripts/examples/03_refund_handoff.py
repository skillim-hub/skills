...
def main():
    ...
    context = CustomerContext(
        name=os.getenv("CUSTOMER_NAME", "דנה לוי"),
        contact=os.getenv("CUSTOMER_CONTACT", "+972501234567"),
        order_number=os.getenv("ORDER_NUMBER", "10493"),
        channel=os.getenv("CHAT_CHANNEL", "chat"),
    )
    ticket = agent.create_ticket(os.getenv("CUSTOMER_MESSAGE", "המוצר הגיע שבור ואני רוצה החזר"), context, env)
