...
def main():
    ...
    def lookup(order_number: str):
        return {
            "status_he": "נשלחה",
            "tracking_number": os.getenv("TRACKING_NUMBER", "IL123456789"),
            "estimated_delivery_date": os.getenv("ESTIMATED_DELIVERY_DATE", "05/06/2026"),
            "late": False,
        }

    agent.order_lookup = lookup
    context = CustomerContext(
        name=os.getenv("CUSTOMER_NAME"),
        contact=os.getenv("CUSTOMER_CONTACT"),
        order_number=os.getenv("ORDER_NUMBER", "10493"),
        channel=os.getenv("CHAT_CHANNEL", "chat"),
    )
    response = agent.reply(os.getenv("CUSTOMER_MESSAGE", "איפה ההזמנה שלי?"), context)
