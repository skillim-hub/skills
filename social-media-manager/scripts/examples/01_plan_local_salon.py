...
def main() -> None:
    args = parse_args()
    platforms = os.getenv("SOCIAL_MEDIA_MANAGER_PLATFORMS", "facebook,instagram").split(",")
    client = SocialMediaManagerClient()
    posts = client.schedule_posts(sample_drafts(platforms, business_type=args.business_type), start_date=args.start_date, days=14)
    dump({"environment": args.env, "scenario": "local_salon", "items": [post.to_dict() for post in posts]})
