...
def main() -> None:
    args = parse_args()
    blocked = os.getenv("SOCIAL_MEDIA_MANAGER_BLACKOUT_DATE", "2026-06-09")
    client = SocialMediaManagerClient(blackout_periods=[BlackoutPeriod(blocked, reason="owner_pause")])
    posts = client.schedule_posts(sample_drafts(["linkedin", "facebook"], business_type=args.business_type), start_date=args.start_date, days=7)
    dump({"environment": args.env, "scenario": "blackout_dates", "blackout_date": blocked, "items": [post.to_dict() for post in posts]})
