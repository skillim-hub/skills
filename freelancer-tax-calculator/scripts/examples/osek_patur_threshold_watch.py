...
args = parser("Osek patur threshold watch.").parse_args()
calc = FreelancerTaxCalculator(load_environment_config(args.env), environment=args.env)
threshold = calc.assess_osek_patur_threshold(os.getenv("FTC_REVENUE_ILS", "116000"))
report = calc.calculate(FreelancerTaxInput(
    business_type="osek-patur",
    annual_revenue_ils=os.getenv("FTC_REVENUE_ILS", "116000"),
    deductible_expenses_ils=os.getenv("FTC_EXPENSES_ILS", "22000"),
    income_tax_advance_rate=os.getenv("FTC_ADVANCE_RATE", "0.06"),
))
print_json({"threshold": threshold, "report": report})
