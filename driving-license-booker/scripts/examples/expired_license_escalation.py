...
def main() -> None:
    ...
    client = DrivingLicenseBookerClient(environment=args.env, state_path=args.state, now=dt.datetime(2026, 6, 4, tzinfo=dt.timezone.utc))
    applicant = Applicant(
    ...
    readiness = client.check_renewal_readiness(applicant=applicant, expiry_date=dt.date.fromisoformat(os.getenv("DLB_EXPIRY_DATE", "2026-01-01")), has_paid_fee=True)
    readiness["escalation"] = "Prepare official renewal check before any driving-related instruction."
    emit(readiness)
