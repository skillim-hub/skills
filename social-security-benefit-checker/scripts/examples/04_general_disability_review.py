...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="General disability review scenario")
...
def main() -> None:
    ...
    profile = ApplicantProfile.from_mapping({'age': 49, 'resident': True, 'employment_status': 'not_working', 'monthly_income': 0, 'medical_disability_percent': 65, 'work_capacity_loss_percent': 75})
    payload = {"scenario": "general_disability_review", "environment": args.env, "result": checker.check_benefit("general_disability", profile).to_dict()}
    if default_city:
