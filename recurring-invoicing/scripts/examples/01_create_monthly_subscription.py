...
def main() -> None:
    ...
    subscription = Subscription(
        subscription_id="SUB-IL-001",
        customer=Customer(name="לקוח לדוגמה בעמ", tax_id="514324995", email="billing@example.co.il"),
        line_items=[LineItem(description="שירות חודשי", quantity="1", unit_price="1200.00")],
        start_date=date(2026, 1, 31),
        interval=Interval.MONTHLY,
    )
    created = client.create_subscription(subscription)
    print_json({"environment": args.env, "subscription": created})
