...
cfg = config(args)
case = RenewalCase(
    kupat_cholim=cfg["default_kupat_cholim"] or "Leumit",
    medication_name="as shown in portal",
    supply_days=5,
    repeats_left=1,
    preferred_fulfillment=Fulfillment.PICKUP,
    patient_alias="parent",
    consent_confirmed=True,
)
dump({
    "config": cfg,
    "triage": PrescriptionRenewalClient().triage(case).to_dict(),
    "checklist": pharmacy_checklist(fulfillment=Fulfillment.PICKUP).to_dict(),
})
