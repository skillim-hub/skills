...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Assess a freelancer client-management workflow.")
...
def main() -> None:
    ...
    payload = {'record_count': 350, 'authorized_users': 1, 'sector': 'professional services', 'purposes': ['client management', 'invoicing'], 'lawful_basis': 'contract', 'processors': ['cloud spreadsheet'], 'retention_months': 84, 'privacy_notice_ready': True}
    payload["business_name"] = f'Freelancer lead list ({args.profile})'
